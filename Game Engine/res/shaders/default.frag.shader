...
void main() {
    ...
    if (dot(normal, L)  < 0.0f){
        ...
        OutputColour += vec4((diffuseColour.rgb * dot(normal, L)) * (tempLightColour * tempLightIntensity), 0.0f); // diffuse
        OutputColour += vec4(diffuseColour.rgb * tempLightColour * pow(max(dot(V, R), 0.0), 32.0f), 0.0f);
    }
