...
namespace IcePickRenderer {
	...
	bool FrameBuffer::Init(Texture colourAttachments[], unsigned int attachmentCount, const Texture& depthAttachment) {
		if (m_Initialised)
			return m_Initialised;
		...
		m_DepthTexID = depthAttachment.GetID();
		glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, m_DepthTexID); // TODO: attachment type can be depth or depth_stencil
